...
while True:
    ...
    for ship in me.get_ships():
        if game.turn_number >= constants.MAX_TURNS - 20:
            end_game = True
            to_shipyard=game_map.get_unsafe_moves(ship.position,me.shipyard.position)
            
            if len(to_shipyard)>0:
                command_queue.append(ship.move(to_shipyard[0])) 
        else:
            ...
            if ship_states[ship.id] == "collecting":
                ...
                # maps with position orders, but also gives us all the surrounding positions
                ...
                if ship.id not in ship_waiting_time:
                    ship_waiting_time[ship.id] = 1
                                
                move = game_map.naive_navigate(ship, me.shipyard.position)
                if move == Direction.Still and ship_waiting_time[ship.id]>25:
                    ship_waiting_time[ship.id] = 1
                    # either at port or blocked by friendly ship or enemy ship move there anyway
                    to_shipyard=game_map.get_unsafe_moves(ship.position,me.shipyard.position)
                    if len(to_shipyard)>0:
                        command_queue.append(ship.move(to_shipyard[0])) 
                else:
    ...
    if len(me.get_ships()) < math.ceil((constants.MAX_TURNS-game.turn_number) / 25) and not end_game:
